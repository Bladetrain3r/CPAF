...
| `iter11_interaction_vs_deviation.py` | **Interaction vs deviation (the noun/verb split, tested hard).** One-way coupling (1→2), so the rig carries its own controls. Below Kc, in a band where the winding number proves the pair is drifting (Ω_norm>0.8) and MI is at floor (~0.06 bits), TE(1→2) is significant at z=100–650 — **directed influence without a deviation**; replicates at a second detuning. Controls hold: K=0 gives z≈−1 (no bias floor), and the null direction TE(2→1) stays z<3 across the drift regime. Two honest corrections: TE is *not* a monotonic ramp — it peaks at the locking onset (K/Kc≈0.85) and declines under strong locking (locked target is self-predictable); and in the locked regime the null direction picks up ~0.0014 bits (z up to 16) — the iter-8 conditioning bias via the lock's lag structure, ~60× below signal (effect size, not z, is the honest null lens at N=100k). Asymmetry: every deviation rides on an interaction, not vice versa | passing |
...
- **Interaction leads deviation; and TE is non-monotonic across locking
  (iter11):** directed influence (TE) is significant on a channel well below
  the locking threshold, where the pair is provably drifting and shares no
  mutual information — so "interaction" (the channel carrying influence) and
  "deviation" (the locking event) are genuinely distinct, with an asymmetry:
  every deviation rides on an interaction, not vice versa. Two corrections the
  experiment forced: (a) TE is *not* a ramp — it peaks at the locking onset
  and declines under strong locking, because a locked target is
  self-predictable so the source adds little new info (a known
  information-dynamics result, rediscovered by being wrong first); (b) the
  no-influence direction is not exactly zero in the locked regime (~0.0014
  bits, z up to 16) — the iter-8 conditioning bias resurfacing through the
  lock's lag structure (θ2 a delayed copy of θ1), ~60× below the forward
  signal. Meta-lesson: at N=100k, z-scores over-report tiny biases; read a
  null by effect size, not significance.
