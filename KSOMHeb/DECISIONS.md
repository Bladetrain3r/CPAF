...
| D7 | Reference math lives in `ksomheb.py`; the browser visualiser ports it to `ksomheb.js` guarded by a Node **parity check** | One source of truth; the JS is verified identical, not assumed | superseded (D25) |
...
| D25 | **Visualiser revamp: the interactive companion runs the verified Python directly** — a stdlib HTTP server (`visualiser/serve.py`) computes trajectories server-side by importing `ksomheb.py` (or a reduction an iteration proved equivalent: ch01's uniform-K mean-field identity, ch07's iter-6 two-oscillator reduction); the browser only renders and scrubs. One tab per textbook chapter, manifest-driven; `selfcheck.py` verifies every non-direct stepper against `ksomheb.py` plus each lab's headline result. The JS port + Node parity check (D7) is retired to `visualiser/legacy/` | The port-and-parity architecture scales with the number of labs, not the ledger: 17 iterations would each need a second JS implementation. Importing the reference implementation leaves no second copy of the math to drift; the suite inherits trust from the verification ledger instead of re-establishing it | locked |
